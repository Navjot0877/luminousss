...
const SignupInput = () => {
  ...
  function password_validate(p) {
    ...
    const spc = new RegExp("^(?=.*[!@#\$%\^&\*])(?=.{8,})");
    

    if(!cap.test(p))
    {
    setFeedback("Enter atleast one captial letter")
    return false
    }

    if(!num.test(p))
    {
    setFeedback("Enter atleast one number")
    return false
    }

    if(!spc.test(p))
    {
    setFeedback("Enter atleast one special character")
    return false
    }
...
<FloatingLabel
      ...
      <label >
        You have an account? Login <Button className="butReg" onClick={(()=> chalLogin())}>Here</Button>
        {/* <span class="hack">Here</span> */}
      </label>
      </div>
